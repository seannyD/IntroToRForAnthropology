...
There's a "cheatsheet" of all the commands we'll use [here](../RCheatSheet.pdf).
...
There's an example of an finished R script in [this file](RScriptExample.R).
...
> **Task**: make a list of booleans where the items are `TRUE` if each corresponding item of `nums` is either 5 or 1. Remember, if you get stuck, you can look up the answers here: [answers](IntroToR_TaskAnswers.html).
