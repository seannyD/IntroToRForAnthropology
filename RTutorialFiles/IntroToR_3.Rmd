...
In this tutorial, we'll load some data from a file. This is as easy as running one line of code, referring to the location of the file on your computer. For example, if I have a file called "WALS_WordOrder.csv" on my desktop, I could load it with this code:
...
> **Task**: Make a boolean vector which is `TRUE` if the latitude of a language is greater than 0, and `FALSE` otherwise.  Assign this to a variable named `northernHemisphere` (create a variable called `northernHemisphere` and store the boolean vector inside it).
