...
map()
# add points
points(d$longitude, d$latitude, col= 4-as.numeric(d$roofShape))
```
...
So, this tells us that the difference between Flat and Non-flat rooves from a random independent sample is `r indepDiff`.  That's more than zero, but is it significantly more than zero?  What would happen if we had chosen a different random selection?  
...
The p value is `r p.value` (or < 1/1000) - so quite significant.  This suggests that the correlation between rainfall and roof shape may not be explained away by Galton's problem.
