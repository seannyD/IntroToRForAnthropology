...
-  How to combine data from different sources using ID names, the `match` function and the `merge` function.

There's a "cheatsheet" of all the commands we use in this course [here](../RCheatSheet.pdf).

# Basic stats
In the last section, we used a database of word orders.  We'll use that again here. Make a new R script in your **analysis** folder and copy this code in:
...
Run these lines to check that they work.

The data frame `d2` now includes variables **latitude** and **family**.

## Mean, standard deviation, minimum and maximum

What is the mean latitude of languages? (Ok, this isn't very interesting, but the point here is to get some experience working with numeric data)
...
(if the code doesn't work, make sure you've installed the 'maps' package by calling `install.packages("maps")`)
